...
export function renderPublishedComponent(component){
 const type=String(component?.type||'Text').toLowerCase();const p=component?.props||{};const text=esc(p.text??p.label??p.content??'');
 if(type==='login')return `<form class="pub-form" data-auth="login"><h2>${esc(p.title||'Log in')}</h2><label class="pub-field"><span>Email</span><input name="email" type="email" required placeholder="Email"></label><label class="pub-field"><span>Password</span><input name="password" type="password" required minlength="6" placeholder="Password"></label><button class="pub-button" type="submit">${esc(p.buttonLabel||'Log in')}</button><small class="pub-form-message" aria-live="polite"></small></form>`;
 if(type==='signup')return `<form class="pub-form" data-auth="signup"><h2>${esc(p.title||'Create account')}</h2><label class="pub-field"><span>Email</span><input name="email" type="email" required placeholder="Email"></label><label class="pub-field"><span>Password</span><input name="password" type="password" required minlength="6" placeholder="Password"></label><button class="pub-button" type="submit">${esc(p.buttonLabel||'Sign up')}</button><small class="pub-form-message" aria-live="polite"></small></form>`;
 if(type==='logout')return `<button class="pub-button" type="button" data-auth="logout">${esc(p.label||'Log out')}</button>`;
 if(type==='user')return `<div class="pub-user" data-auth="user">Loading user…</div>`;
 ...
 if(type==='forms'||type==='form'){const fields=Array.isArray(p.fields)&&p.fields.length?p.fields:['Name','Email','Message'];const inputs=fields.map((field,index)=>{const f=safeField(field,index);const tag=f.name==='message'||f.name==='description'?'textarea':'input';const attrs=tag==='textarea'?`name="${esc(f.name)}" rows="4" placeholder="${esc(f.label)}"`:`name="${esc(f.name)}" type="${f.name==='email'?'email':'text'}" placeholder="${esc(f.label)}"`;return `<label class="pub-field"><span>${esc(f.label)}</span><${tag} ${attrs} required></${tag}></label>`}).join('');return `<form class="pub-form"><h2>${esc(p.title||'Contact us')}</h2>${inputs}<button class="pub-button" type="submit">${esc(p.submitLabel||'Submit')}</button><small class="pub-form-message" aria-live="polite"></small></form>`}
