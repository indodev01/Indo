...
export function renderAuthComponent(component, mount, { onSuccess=()=>{}, onError=()=>{} }={}) {
  ...
  if (type==='Login' || type==='Signup') {
    ...
    const password=document.createElement('input'); password.name='password'; password.type='password'; password.required=true; password.minLength=6; password.placeholder='Password';
    const submit=document.createElement('button'); submit.type='submit'; submit.textContent=type==='Login'?'Log in':'Sign up';
    const message=document.createElement('small');
    form.append(title,email,password,submit,message); mount.appendChild(form);
