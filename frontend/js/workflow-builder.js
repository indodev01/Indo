...
function renderConditions(container,w){
  const conditions=Array.isArray(w.conditions)?w.conditions:[];w.conditions=conditions;container.innerHTML='';
...
async function init(){
  ...
  state.project=r.data;state.workflows=Array.isArray(r.data.app_definition?.workflows)?structuredClone(r.data.app_definition.workflows):[];state.workflows.forEach(w=>{w.actions=(Array.isArray(w.actions)&&w.actions.length?w.actions:[normalizeAction({type:'show-message',message:'Done'})]).map(normalizeAction)});if(!state.workflows.length)state.workflows=[defaultWorkflow()];$('projectName').textContent=r.data.name;renderCurrent();
}
