...
async function executeAction(action,context={}){if(!action)return;
  ...
  else if(action.type==='database-create'){if(!action.tableId)throw new Error('Database table is required');const row=await DataBinding.create(action.tableId,action.data||context.formData||{});window.dispatchEvent(new CustomEvent('indo:database-created',{detail:row}));return row}
}
