...
function readPosition(node, component) {
  ...
  if (responsive && Number.isFinite(Number(responsive.x)) && Number.isFinite(Number(responsive.y))) return { x:Number(responsive.x), y:Number(responsive.y), width:Number(responsive.width)||Math.round(node.getBoundingClientRect().width||160), height:Number(responsive.height)||Math.round(node.getBoundingClientRect().height||48) };
  const legacy = component?.props?.position;
  if (legacy && activeDevice === 'desktop') return { x:Number(legacy.x)||0, y:Number(legacy.y)||0, width:Number(legacy.width)||Math.round(node.getBoundingClientRect().width||160), height:Number(legacy.height)||Math.round(node.getBoundingClientRect().height||48) };
  const existing = node.__responsiveLayout;
  if (existing && node.__responsiveDevice === activeDevice) return clone(existing);
  const canvasRect = canvas.getBoundingClientRect(); const rect = node.getBoundingClientRect();
  const rawX = rect.left - canvasRect.left + canvas.scrollLeft; const rawY = rect.top - canvasRect.top + canvas.scrollTop;
  const width = Math.max(80, Math.round(rect.width || 160)); const height = Math.max(36, Math.round(rect.height || 48));
  const targetWidth = Math.max(280, Math.min(DEVICE_DEFAULT_WIDTHS[activeDevice] || 900, canvas.clientWidth || 900));
  return { x:Math.max(12, Math.min(Math.round(rawX), Math.max(12,targetWidth-width-12))), y:Math.max(12,Math.round(rawY)), width, height };
}
...
function enhance(){
  if(!canvas||!definition)return;const page=currentPage();const nodes=[...canvas.querySelectorAll('.canvas-item')];
  ...
  nodes.filter(n=>n.classList.contains('canvas-header-component')).forEach(node=>{const component=componentForNode(node),bucket=deviceBucket(component);if(!bucket)return;const header=node.querySelector('.app-header');if(!header)return;const title=header.querySelector('.header-title-wrap'),menu=header.querySelector('.header-menu-toggle');if(title){const p=bucket.titlePosition;if(p)title.style.transform=`translate(${Number(p.x)||0}px,${Number(p.y)||0}px)`;title.style.touchAction='none';}if(menu){const p=bucket.menuPosition;if(p)menu.style.transform=`translate(${Number(p.x)||0}px,${Number(p.y)||0}px)`;menu.style.touchAction='none';}});
}
